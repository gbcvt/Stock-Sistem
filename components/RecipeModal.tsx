...
const RecipeModal: React.FC<RecipeModalProps> = ({ isOpen, onClose, onSave, recipeToEdit, products }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-60 z-50 flex justify-center items-center p-4 transition-opacity duration-300">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-2xl relative max-h-[90vh] overflow-y-auto flex flex-col">
        ...
        <form onSubmit={handleSubmit} className="flex-grow flex flex-col">
          <div className="p-6 space-y-6 flex-grow">
            <div>
              <label htmlFor="name" className="block text-sm font-medium text-slate-700">Nome da Receita</label>
            ...
            <div>
              <h4 className="text-sm font-medium text-slate-700">Ingredientes</h4>
                ...
                {ingredients.map((ing, index) => (
                  <div key={index} className="flex items-center gap-2">
                    <select
                      ...
                      className="w-1/2 border-slate-300 rounded-md shadow-sm focus:ring-orange-500 focus:border-orange-500"
                    >
              ...
              <button type="button" onClick={handleAddIngredient} className="mt-2 inline-flex items-center px-3 py-1.5 border border-transparent text-sm font-medium rounded-md text-orange-700 bg-orange-100 hover:bg-orange-200">
                <PlusIcon className="w-4 h-4 mr-2"/> Adicionar Ingrediente
              </button>
            </div>
             <div>
              <label htmlFor="instructions" className="block text-sm font-medium text-slate-700">Instruções (Opcional)</label>
              <textarea id="instructions" value={instructions} onChange={e => setInstructions(e.target.value)} rows={5} className="mt-1 block w-full border-slate-300 rounded-md shadow-sm focus:ring-orange-500 focus:border-orange-500"/>
            </div>
          </div>
